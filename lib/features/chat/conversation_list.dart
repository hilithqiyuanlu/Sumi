            ...
            Expanded(
              child: conversations.isEmpty
                  ...
                  : ListView.separated(
                      ...
                      separatorBuilder: (_, _) => const Divider(height: 1, indent: s16),
                      itemBuilder: (context, index) {
                        final conv = conversations[index];
                        final isActive = conv.id == store.currentConversationId;

                        return ListTile(
                          selected: isActive,
                          selectedTileColor: mint.withValues(alpha: 0.2),
                          title: Text(
                          ...
                          onTap: () {
                            Navigator.pop(context);
                            if (!isActive) store.switchConversation(conv.id);
                          },
                          onLongPress: () {
                            _showDeleteDialog(context, conv.id, conv.title);
                          },
                        );
                      },
                    ),
